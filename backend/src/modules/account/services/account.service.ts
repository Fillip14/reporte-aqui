...
import { deleteProfileService, patchProfileService } from '../../profile/services/profile.service';
...
import { ProfileData, ProfileUpdate } from '../schemas/account.schema';
...
export const patchAccountService = async (userData: ProfileUpdate, user_id: string) => {
  const { email, ...data } = userData;

  await patchUserService(email, user_id);
  await patchProfileService(data, user_id);

  return;
};

export const deleteAccountService = async (userData: ProfileData) => {
